...
const createAddress = asyncHandler(async (req, res) => {
  res.success(await addresses.createAddress(req.user.id, req.valid.body), 201)
})
...
const createRental = asyncHandler(async (req, res) => {
  res.success(await rentals.createRental(req.user.id, req.valid.body), 201)
})
...
const createReview = asyncHandler(async (req, res) => {
  res.success(await reviews.upsertReview(req.user.id, req.valid.body), 201)
})
